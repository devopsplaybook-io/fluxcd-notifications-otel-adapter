...
A key concept of fluxcd-notifications-otel-adapter are processors. These are modules that can be added to hande new type of URL.

Built-in processors are stored in the [fluxcd-notifications-otel-adapter-server/processors-system](../../fluxcd-notifications-otel-adapter-server/processors-system/) of thise repository.

However, users can implement their own processors wihout changing the application.

Do do so you need to create a Javascript file in the `/opt/app/fluxcd-notifications-otel-adapter/processors-user` of the runtime container
...
It is recommenede to have a look at the existing [System Processors](../../fluxcd-notifications-otel-adapter-server/processors-system/).
